...
class OsBuildReader(private val properties: SystemProperties) {

    fun read(): OsBuild = OsBuild(
    ...
    private fun detectRom(): CustomRom? = KNOWN_ROMS.firstNotNullOfOrNull { (key, name) ->
        properties[key]?.let { CustomRom(name, it) }
    }

    fun readBootState(): BootState = BootState(
